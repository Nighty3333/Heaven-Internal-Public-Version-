...
const MENU_KEYS: [(&str, imgui::Key); 14] = [
...
impl HeavenOverlay {
    ...
    fn draw_menu(&mut self, ui: &Ui) {
        ...
        let tabs: Vec<&str> = menu.iter().map(|t| t.name).collect();
                                ...
                                for c in &sec.controls {
                                    match c {
                                        ...
                                        Ctrl::Custom(Custom::AboutLayout) => {
                                            ...
                                            ui.same_line();
                                            let ki = menu_key_idx();
                                            if btn(ui, "##menukey", MENU_KEYS[ki].0) {
                                                crate::settings::set_toggle_key(((ki + 1) % MENU_KEYS.len()) as u32);
                                            }
                                            ui.same_line();
                                            ui.text_colored(DIM, "(click to cycle)");
                                            ui.dummy([0.0, 8.0]);
                            ...
                            for c in &sec.controls {
                                match c {
                                    ...
                                    Ctrl::Custom(Custom::AboutLayout) => {
                                        ...
                                        ui.text_colored(DIM, "Open / close key");
                                        ui.same_line();
                                        if ui.button(&format!("{}##menukeyc", MENU_KEYS[ki].0)) {
                                            crate::settings::set_toggle_key(((ki + 1) % MENU_KEYS.len()) as u32);
                                        }
                                    }
                                    Ctrl::Custom(Custom::Credits) => {
